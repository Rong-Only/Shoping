...
class _HomeState extends State<Home> {
  ...
  void _getCategory() async {
    ...
    try {
      ApiRespone response = await getCategory();

      if (response.error == null && response.data != null) {
        List<Category> categoryList = (response.data as List).cast<Category>();

        setState(() {
          _itemCategory = categoryList;
          _hasError = false;
        });
      } else {
        setState(() {
...
Widget buildSectionTitle(String title) {
  return Padding(
      padding: const EdgeInsets.only(left: 15, right: 15), child: kRow(title));
}
  ...
  return SizedBox(
    ...
    child: ListView.builder(
      ...
      itemBuilder: (context, index) {
        var category = _itemCategory[index];
